...
Процедура ЗаписатьИсключенияВРС() Экспорт
	...
	Для Каждого СтрокаЗаписиЖР Из ТаблицаЗаписей Цикл
		...
		СохраненныйЛогЗапроса = ЗначениеИзСтрокиJSON(СтрокаЗаписиЖР.Данные, Ложь, "Дата"); // см. РегистрыСведений.ЛогиТрафика.НовыйЛогЗапроса
...
Функция НовоеHTTPСоединениеПоПараметрам(ПараметрыЗапроса, ТекстОшибки = """");
	...
	Попытка
		...
		Возврат Соединение;
	Исключение
		// TODO сделать запись в ЖР а так же оповещение пользователю иснтрумента
...
Функция ДанныеДляЗаписиИсключения(ЛогЗапроса, ЗаписыватьТело) Экспорт
	...
	ДанныеДляЗаписиИсключения = ЗначениеВJSON(ЗаписьЖурналаРегистрации);
