...
Функция ЗагрузитьНастройкиЛогирования() Экспорт
	...
	ЗагруженныеНастройкиЛогирование = ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, "Настройки", НастройкиЛогирование);// Произвольный
	...
	НастройкиЛогирование.Редактор = НастройкиРедактораИзХранилища(ЗагруженныеНастройкиЛогирование);
...
Функция ЗагрузитьНастройкиРедактора() Экспорт
	...
	Возврат ЗагрузитьНастройкиЛогирования().Редактор;
	
КонецФункции

// Сохраняет настройки Monaco Editor без потери остальных настроек логирования.
//
// Параметры:
...
Процедура СохранитьНастройкиРедактора(НастройкиРедактора) Экспорт
	...
	ПолныеНастройки = ЗагрузитьНастройкиЛогирования();
	...
	КонецЕсли;
	ПолныеНастройки.Редактор = Редактор;
	СохранитьНастройкиЛогирования(ПолныеНастройки);
	
КонецПроцедуры

// Эффективные флаги логирования с учетом иерархии родительских настроек.
// Сохраненные дочерние значения не меняются: при выключенном родителе они дают Ложь.
//
// Параметры:
//  Настройки - Структура, Неопределено - см. НовыйНастройкиЛогирования; если Неопределено - загружаются
//
// Возвращаемое значение:
//  Структура:
// * ВключеноЛогирование - Булево
...
// * ЗаписыватьВРСТело - Булево
// * ЗаписыватьВЖРТело - Булево
// * МаксимальныйРазмерТела - Число - 0, если тело никуда не пишется
//
Функция ЭффективныеНастройкиЛогирования(Настройки = Неопределено) Экспорт
	
	Если Настройки = Неопределено Тогда
		Настройки = ЗагрузитьНастройкиЛогирования();
	КонецЕсли;
	
	ВключеноЛогирование = Настройки.ВключеноЛогирование;
	ЗаписыватьВРС = ВключеноЛогирование И Настройки.ЗаписыватьВРС;
	ЗаписыватьВЖР = ВключеноЛогирование И Настройки.ЗаписыватьВЖР;
	ЗаписыватьВРСТело = ЗаписыватьВРС И Настройки.ЗаписыватьВРСТело;
	ЗаписыватьВЖРТело = ЗаписыватьВЖР И Настройки.ЗаписыватьВЖРТело;
	
	МаксимальныйРазмерТела = 0;
	Если ЗаписыватьВРСТело Или ЗаписыватьВЖРТело Тогда
		МаксимальныйРазмерТела = Настройки.МаксимальныйРазмерТела;
	КонецЕсли;
	
	Результат = Новый Структура;
	Результат.Вставить("ВключеноЛогирование", ВключеноЛогирование);
	Результат.Вставить("ЗаписыватьВРС", ЗаписыватьВРС);
	Результат.Вставить("ЗаписыватьВЖР", ЗаписыватьВЖР);
	Результат.Вставить("ЗаписыватьВРСТело", ЗаписыватьВРСТело);
	Результат.Вставить("ЗаписыватьВЖРТело", ЗаписыватьВЖРТело);
	Результат.Вставить("МаксимальныйРазмерТела", МаксимальныйРазмерТела);
	
	Возврат Результат;
	
КонецФункции

// Признак эффективной записи в регистр сведений.
//
// Возвращаемое значение:
//  Булево
//
Функция ЗаписыватьВРС() Экспорт
	
	Возврат ЭффективныеНастройкиЛогирования().ЗаписыватьВРС;
	
КонецФункции

// Признак эффективной записи в журнал регистрации.
//
// Возвращаемое значение:
//  Булево
//
Функция ЗаписыватьВЖР() Экспорт
	
	Возврат ЭффективныеНастройкиЛогирования().ЗаписыватьВЖР;
	
КонецФункции

// Признак записи тела запроса/ответа в регистр сведений.
//
// Возвращаемое значение:
//  Булево
//
Функция ЗаписыватьТелоВРС() Экспорт
	
	Возврат ЭффективныеНастройкиЛогирования().ЗаписыватьВРСТело;
	
КонецФункции

// Признак записи тела в журнал регистрации.
//
// Возвращаемое значение:
//  Булево
//
Функция ЗаписыватьТелоВЖР() Экспорт
	
	Возврат ЭффективныеНастройкиЛогирования().ЗаписыватьВЖРТело;
	
КонецФункции

// Признак сохранения тела HTTP-запроса при формировании лога.
//
// Возвращаемое значение:
//  Булево
//
Функция ЗаписыватьТелоЗапроса() Экспорт
	
	ЭффективныеНастройки = ЭффективныеНастройкиЛогирования();
	Возврат ЭффективныеНастройки.ЗаписыватьВРСТело Или ЭффективныеНастройки.ЗаписыватьВЖРТело;
...
Процедура СохранитьНастройкиЛогирования(Настройки) Экспорт
	...
	Если ТипЗнч(ЗагруженныеНастройки) = Тип("Структура") Тогда
		...
		ПолныеНастройки.Редактор = НастройкиРедактораИзХранилища(ЗагруженныеНастройки);
	КонецЕсли;
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		Если Настройки.Свойство("Редактор") И ТипЗнч(Настройки.Редактор) = Тип("Структура") Тогда
			Редактор = OneSniffer_РедакторКодаКлиентСервер.НастройкиРедактораПоУмолчанию();
			ЗаполнитьЗначенияСвойств(Редактор, Настройки.Редактор);
			ПолныеНастройки.Редактор = Редактор;
		КонецЕсли;
	КонецЕсли;
	
	ХранилищеОбщихНастроекСохранить(КлючОбъекта, "Настройки", ПолныеНастройки);
...
// Извлекает тело HTTP-ответа с учётом gzip, text и сохранения в файл.
//
// Параметры:
//  HTTPОтвет - HTTPОтвет
...
//
// Возвращаемое значение:
//  ДвоичныеДанные, Строка, Неопределено
//
Функция ТелоHTTPОтвета(HTTPОтвет, Знач ИмяВыходногоФайла = "") Экспорт
	...
	ЗаголовокТипОтвета = HTTPОтвет.Заголовки.Получить("Content-Type");
	
	Если ЗаголовокШифрованияGzip = "gzip" Тогда
	...
	ИначеЕсли СтрНайти(ЗаголовокТипОтвета, "text") Тогда
		
		ТелоСтрокой = HTTPОтвет.ПолучитьТелоКакСтроку();
		Если ПустаяСтрока(ТелоСтрокой) Тогда
			Возврат "";
		КонецЕсли;
		Возврат ТелоСтрокой;
...
Функция ЗаписатьИсключенияВРС() Экспорт
	...
	Если ТаблицаЗаписей.Количество() > 0 Тогда
		Константы.OneSniffer_ДатаЧтенияЖР.Установить(МаксимальнаяДатаЖурнала);
	КонецЕсли;
...
Функция ПолучитьДетализациюПротоколаЗапроса(КлючЗаписи) Экспорт
	...
	КонецЕсли;
	ПротоколЗапроса.ПредставлениеТелоЗапроса = OneSniffer_РедакторКодаКлиентСервер.УсечьТекстДляОтображения(
		ПротоколЗапроса.ПредставлениеТелоЗапроса);
	...
	КонецЕсли;
	ПротоколЗапроса.ПредставлениеТелоОтвета = OneSniffer_РедакторКодаКлиентСервер.УсечьТекстДляОтображения(
		ПротоколЗапроса.ПредставлениеТелоОтвета);
...
Функция ПолучитьТекстЛога(КлючЗаписи, ВключатьОтвет = Ложь, БезАвторизации = Ложь, ДляОтображения = Истина) Экспорт
	...
	Если ДляОтображения Тогда
		РезультатТекст = OneSniffer_РедакторКодаКлиентСервер.УсечьТекстДляОтображения(РезультатТекст);
	КонецЕсли;
...
Функция ПодготовитьЛог(ЛогЗапроса, Назначение) Экспорт
	...
	Если Назначение = Перечисления.НазначениеЛога.Журнал Тогда
		...
		Если Не ЭффективныеНастройки.ЗаписыватьВЖРТело Тогда
			Результат.Содержание.ТелоЗапроса = "";
			Результат.Содержание.ТелоОтвета = "";
		Иначе
			ЛимитТелаКБ = ЭффективныеНастройки.МаксимальныйРазмерТела;
	...
	ИначеЕсли Назначение = Перечисления.НазначениеЛога.Регистр Тогда
		...
		Если ЭффективныеНастройки.ЗаписыватьВРСТело Тогда
			ЛимитТелаКБ = ЭффективныеНастройки.МаксимальныйРазмерТела;
			...
			Если ТипЗнч(ТелоОтвета) = Тип("ДвоичныеДанные") Тогда
				Результат.РазмерОтвета = ТелоОтвета.Размер() / 1000;
			КонецЕсли;
			Результат.Содержание.ТелоЗапроса = УсечьТелоПоЛимитуКБ(
				Результат.Содержание.ТелоЗапроса,
				ЛимитТелаКБ,
...
Процедура ДополнитьЛогДаннымиОтвета(Результат, ЛогЗапроса)
	...
	Если ЛогЗапроса.Свойство("HTTPОтвет", HTTPОтвет) И HTTPОтвет <> Неопределено Тогда
		...
		ЭффективныеНастройки = ЭффективныеНастройкиЛогирования();
		Если ЭффективныеНастройки.ЗаписыватьВЖРТело Или ЭффективныеНастройки.ЗаписыватьВРСТело Тогда
			ТелоОтвета = ТелоHTTPОтвета(HTTPОтвет);
	...
	ИначеЕсли ЛогЗапроса.Свойство("Входящий") И ЛогЗапроса.Входящий
		И ЗначениеЗаполнено(ЛогЗапроса.КодОтвета)
		И ТипЗнч(Результат.Содержание) = Тип("Структура")
		И ТипЗнч(Результат.Содержание.ТелоОтвета) = Тип("ДвоичныеДанные")
		И Результат.Содержание.ТелоОтвета.Размер() > 0 Тогда
		
		Результат.РазмерОтвета = Результат.Содержание.ТелоОтвета.Размер() / 1000;
