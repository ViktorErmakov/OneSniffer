...
Функция ЗаписатьИсключенияВРС() Экспорт
	...
	Если КонстантаСуществует Тогда
		...
		ДатаНачалаКонстанта = Константы.OneSniffer_ДатаЧтенияЖР.Получить();
		Если ЗначениеЗаполнено(ДатаНачалаКонстанта) Тогда
			ДатаНачала = ДатаНачалаКонстанта;
		КонецЕсли;
	...
	Если ТаблицаЗаписей.Количество() > 0 И КонстантаСуществует Тогда
...
Процедура ДополнитьЛогДаннымиОтвета(Результат, ЛогЗапроса, Настройки, Знач ИмяВыходногоФайла = "")
	...
	Если ЛогЗапроса.Свойство("HTTPОтвет", HTTPОтвет) И HTTPОтвет <> Неопределено Тогда
		...
		Если Настройки.ЗаписыватьВЖРТело Или Настройки.ЗаписыватьВРСТело Тогда
			...
			Если ТипЗнч(ТелоОтвета) = Тип("ДвоичныеДанные") И ТелоОтвета.Размер() > 0 Тогда
				Результат.РазмерОтвета = ТелоОтвета.Размер() / 1000;
			КонецЕсли;
