...
//
// Возвращаемое значение:
//  Структура, Неопределено - см. РегистрыСведений.ЛогиТрафика.НовыйЛогЗапроса
//
Функция ЛогЗапросаИзДанныхЖурнала(ДанныеЖурнала) Экспорт
	...
	СодержаниеЛога = РегистрыСведений.ЛогиТрафика.НормализоватьСодержаниеЗапроса(Неопределено);
	...
	КонецЕсли;
	НормализоватьСодержаниеЛогаИзЖурнала(СодержаниеЛога);
	ЛогЗапроса.Содержание = СодержаниеЛога;
...
//
Процедура НормализоватьСодержаниеЛогаИзЖурнала(Содержание)
	
	Если ТипЗнч(Содержание) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	ЗаголовкиЗапроса = Неопределено;
	Если Содержание.Свойство("ЗаголовкиЗапроса", ЗаголовкиЗапроса)
		И ТипЗнч(ЗаголовкиЗапроса) = Тип("Строка")
		И Не ПустаяСтрока(ЗаголовкиЗапроса) Тогда
		Содержание.ЗаголовкиЗапроса = ЗначениеИзСтрокиJSON(ЗаголовкиЗапроса, Истина);
	КонецЕсли;
	
	ЗаголовкиОтвета = Неопределено;
	Если Содержание.Свойство("ЗаголовкиОтвета", ЗаголовкиОтвета)
		И ТипЗнч(ЗаголовкиОтвета) = Тип("Строка")
		И Не ПустаяСтрока(ЗаголовкиОтвета) Тогда
		Содержание.ЗаголовкиОтвета = ЗначениеИзСтрокиJSON(ЗаголовкиОтвета, Истина);
	КонецЕсли;
	
	Нормализованное = РегистрыСведений.ЛогиТрафика.НормализоватьСодержаниеЗапроса(Содержание);
	ЗаполнитьЗначенияСвойств(Содержание, Нормализованное);
...
Процедура ЗагрузитьЛогиHTTPИзФайла(Знач Адрес, Отказ = Ложь, ТекстОшибки = "") Экспорт
	...
	Для Каждого ПротоколЗапроса Из ПротоколыЗапросов Цикл
		...
		ЗаполнитьЗначенияСвойств(Запись, ПротоколЗапроса);
		Содержание = РегистрыСведений.ЛогиТрафика.НовыйСодержаниеЗапроса();
...
Функция ДанныеДляЗаписиИсключения(ЛогЗапроса) Экспорт
	...
	ЗаписьЖурналаРегистрации = ОбщегоНазначения.СкопироватьРекурсивно(ЛогЗапроса); // см. РегистрыСведений.ЛогиТрафика.НовыйЛогЗапроса
	//@skip-warning
	ЗаписьЖурналаРегистрации.Метод = Строка(ЗаписьЖурналаРегистрации.Метод);
	
	ЗаголовкиЗапроса = ОбщегоНазначения.СкопироватьРекурсивно(ЗаписьЖурналаРегистрации.Содержание.ЗаголовкиЗапроса);
	МаскироватьСекретныеЗаголовки(ЗаголовкиЗапроса);
	//@skip-warning
	ЗаписьЖурналаРегистрации.Содержание.ЗаголовкиЗапроса = ЗначениеВJSON(ЗаголовкиЗапроса);
	
	ЗаголовкиОтвета = ОбщегоНазначения.СкопироватьРекурсивно(ЗаписьЖурналаРегистрации.Содержание.ЗаголовкиОтвета);
	МаскироватьСекретныеЗаголовки(ЗаголовкиОтвета);
	//@skip-warning
	ЗаписьЖурналаРегистрации.Содержание.ЗаголовкиОтвета = ЗначениеВJSON(ЗаголовкиОтвета);
	
	ЗаписьЖурналаРегистрации.Длительность = (ТекущаяУниверсальнаяДатаВМиллисекундах() - ЛогЗапроса.ВремяНачала)/1000;
	
	ЗаписыватьТело = ЗаписыватьТелоВЖР();
	...
	Если ЗаписыватьТело Тогда
			...
			Если ДвоичныеДанныеЗапроса <> Неопределено Тогда
				...
				ТелоЗапроса = ПолучитьСтрокуИзДвоичныхДанных(ДвоичныеДанныеЗапроса);
				ЗаписьЖурналаРегистрации.Содержание.ТелоЗапроса = ТелоЗапроса;
				
			КонецЕсли;
			
	КонецЕсли;
	
	Если ЛогЗапроса.Свойство("HTTPОтвет") И ЛогЗапроса.HTTPОтвет <> Неопределено Тогда
		
		HTTPОтвет = ЛогЗапроса.HTTPОтвет;
		ЗаписьЖурналаРегистрации.КодОтвета           = HTTPОтвет.КодСостояния;
		ЗаголовкиОтветаHTTP = ОбщегоНазначения.СкопироватьРекурсивно(HTTPОтвет.Заголовки);
		МаскироватьСекретныеЗаголовки(ЗаголовкиОтветаHTTP);
		//@skip-warning
		ЗаписьЖурналаРегистрации.Содержание.ЗаголовкиОтвета = ЗначениеВJSON(ЗаголовкиОтветаHTTP);
		...
		Если ЗаписыватьТело Тогда
			...
			Если ДвоичныеДанныеОтвета <> Неопределено Тогда
				...
				ЗаписьЖурналаРегистрации.РазмерОтвета = ДвоичныеДанныеОтвета.Размер()/1000;
				
				ТелоОтвета = ПолучитьСтрокуИзДвоичныхДанных(ДвоичныеДанныеОтвета);
				ЗаписьЖурналаРегистрации.Содержание.ТелоОтвета = ТелоОтвета;
				
			КонецЕсли;
			
		КонецЕсли;
		
		ЗаписьЖурналаРегистрации.Удалить("HTTPОтвет");
		
	КонецЕсли;
	
	ДанныеДляСериализации = РегистрыСведений.ЛогиТрафика.НовыйСтруктураМенеджераЗаписи();
	ЗаполнитьЗначенияСвойств(ДанныеДляСериализации, ЗаписьЖурналаРегистрации);
	ДанныеДляСериализации.Метод = Строка(ЗаписьЖурналаРегистрации.Метод);
	ДанныеДляСериализации.Содержание = РегистрыСведений.ЛогиТрафика.НормализоватьСодержаниеЗапроса(
		ЗаписьЖурналаРегистрации.Содержание);
	
	ДанныеДляЗаписиИсключения = ЗначениеВJSON(ДанныеДляСериализации);
