...
Функция ЗаписыватьТелоВРС() Экспорт
	...
	НастройкиЛогирования = ЗагрузитьНастройкиЛогирования();
	Возврат НастройкиЛогирования.ЗаписыватьВРС И НастройкиЛогирования.ЗаписыватьВРСТело;
...
Функция ЗаписыватьТелоВЖР() Экспорт
	...
	НастройкиЛогирования = ЗагрузитьНастройкиЛогирования();
	Возврат НастройкиЛогирования.ЗаписыватьВЖР И НастройкиЛогирования.ЗаписыватьВЖРТело;
...
Функция ЗаписыватьТелоЗапроса() Экспорт
	...
	Возврат ЗаписыватьТелоВРС() Или ЗаписыватьТелоВЖР();
...
Функция ПодготовитьЛог(ЛогЗапроса, Назначение) Экспорт
	...
	Если Назначение = Перечисления.НазначениеЛога.Журнал Тогда
		...
		Если Не ЗаписыватьТелоВЖР() Тогда
	...
	ИначеЕсли Назначение = Перечисления.НазначениеЛога.Регистр Тогда
		...
		Если ЗаписыватьТелоВРС() Тогда
			...
			Результат.Содержание.ТелоОтвета = УсечьBase64ИзображенийВТелеОтвета(ТелоОтвета);
			Если ТипЗнч(Результат.Содержание.ТелоОтвета) = Тип("ДвоичныеДанные") Тогда
...
Процедура ДополнитьЛогДаннымиОтвета(Результат, ЛогЗапроса)
	...
	Если ЛогЗапроса.Свойство("HTTPОтвет", HTTPОтвет) И HTTPОтвет <> Неопределено Тогда
		...
		Если ЗаписыватьТелоВЖР() Или ЗаписыватьТелоВРС() Тогда
...
Функция УсечьBase64ИзображенийВТелеОтвета(ТелоОтвета)
	
	Если ТелоОтвета = Неопределено Или ТипЗнч(ТелоОтвета) <> Тип("ДвоичныеДанные") Тогда
		Возврат ТелоОтвета;
	КонецЕсли;
	
	ТелоОтветаСтрока = ПолучитьСтрокуИзДвоичныхДанных(ТелоОтвета, КодировкаТекста.UTF8);
	Если ПустаяСтрока(ТелоОтветаСтрока) Тогда
		Возврат ТелоОтвета;
	КонецЕсли;
	
	ТелоОтветаJSON = ЗначениеИзСтрокиJSON(ТелоОтветаСтрока);
	Если ТипЗнч(ТелоОтветаJSON) <> Тип("Соответствие") Тогда
		Возврат ТелоОтвета;
	КонецЕсли;
	
	ПорогУсечения = 256 * 1024; // мелкие иконки оставляем для превью
	ЕстьИзменения = Ложь;
	
	Для Каждого СтрокаТелаОтвета Из ТелоОтветаJSON Цикл
		Если СтрокаТелаОтвета.Ключ = "image"
			И ТипЗнч(СтрокаТелаОтвета.Значение) = Тип("Строка")
			И СтрДлина(СтрокаТелаОтвета.Значение) > ПорогУсечения Тогда
	...
	ТелоОтветаСтрока = ЗначениеВJSON(ТелоОтветаJSON);
	Возврат ПолучитьДвоичныеДанныеИзСтроки(ТелоОтветаСтрока);
...
Функция ИзвлечьПревьюКартинок(Тело, Заголовки) Экспорт
	...
	Если ПустаяСтрока(ТекстТела) Или СтрНайти(НРег(ТипКонтента), "json") = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
	Попытка
		ДанныеJSON = ЗначениеИзСтрокиJSON(ТекстТела);
	Исключение
		Возврат Результат;
	КонецПопытки;
