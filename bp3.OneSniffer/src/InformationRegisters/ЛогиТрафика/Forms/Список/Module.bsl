...
Процедура ВыполнитьПроизвольныйЗапрос(Команда)
	...
	Отказ = Ложь;
	ТекстОшибки = "";
	ТекстЗапроса = ПроизвольныйЗапрос.ПолучитьТекст();
...
Процедура ТестовыйЗапрос(Команда)
	...
	ПроизвольныйЗапрос.Очистить();
	ПроизвольныйЗапрос.УстановитьТекст(ТекстТестовогоЗапросаКартинка());
	ПоказыватьПроизвольныйЗапрос = Истина;
	ИзменитьВидимостьДоступность();
...
Процедура ТестовыйЗапросОшибка(Команда)
	...
	ПроизвольныйЗапрос.Очистить();
	ПроизвольныйЗапрос.УстановитьТекст(ТекстТестовогоЗапросаОшибка());
	ПоказыватьПроизвольныйЗапрос = Истина;
	ИзменитьВидимостьДоступность();
...
Процедура ТестовыйЗапросАвторизация(Команда)
	...
	ПроизвольныйЗапрос.Очистить();
	ПроизвольныйЗапрос.УстановитьТекст(ТекстТестовогоЗапросаАвторизация());
	ПоказыватьПроизвольныйЗапрос = Истина;
	ИзменитьВидимостьДоступность();
...
Процедура ТестовыйЗапросВходящийHTTP(Команда)
	...
	ПроизвольныйЗапрос.Очистить();
	ПроизвольныйЗапрос.УстановитьТекст(ТекстТестовогоЗапросаВходящийHTTP());
	ПоказыватьПроизвольныйЗапрос = Истина;
	ИзменитьВидимостьДоступность();
...
//	Протокол = ?(ТекущиеДанные.Порт = 443, "https://", "http://");
	...
	Если ПротоколЗапрос.ДеревоЗапроса <> Неопределено Тогда
		...
		Элементы.СтраницаЗапросаJSON.Картинка = БиблиотекаКартинок.OneSniffer_ДеревоДанных;
	Иначе
		ЗапросJSON.ПолучитьЭлементы().Очистить();
		Элементы.СтраницаЗапросаJSON.Картинка = Новый Картинка;
	КонецЕсли;
	...
	Если ПротоколЗапрос.ДеревоОтвета <> Неопределено Тогда
		...
		Элементы.СтраницаОтветJSON.Картинка = БиблиотекаКартинок.OneSniffer_ДеревоДанных;
	Иначе
		ОтветJSON.ПолучитьЭлементы().Очистить();
		Элементы.СтраницаОтветJSON.Картинка = Новый Картинка;
	КонецЕсли;
...
&НаСервере
Функция ПрочитатьИсключенияИзЖР(Принудительно = Ложь)
	
	ДлительнаяОперацияИсключенияЖР = Неопределено;
	...
	Если Не НастройкиЛогирования.ВключеноЛогирование Тогда
		Возврат Новый Структура("ДлительнаяОперация, КоличествоИмпортированных", ДлительнаяОперацияИсключенияЖР, 0);
	КонецЕсли;
	...
	Если Принудительно Тогда
		...
		КоличествоИмпортированных = ЛогированиеТрафика.ЗаписатьИсключенияВРС();
		
	Иначе
		
		Если Не НастройкиЛогирования.ПолучениеИсключенийВключено Тогда
			Возврат Новый Структура("ДлительнаяОперация, КоличествоИмпортированных", ДлительнаяОперацияИсключенияЖР, 0);
		КонецЕсли;
		
		Если НастройкиЛогирования.ПолучатьИсключенияФЗ Тогда
			ДлительнаяОперацияИсключенияЖР = ЛогированиеТрафика.ЗаписатьИсключенияВРСВФоне();
		Иначе
			КоличествоИмпортированных = ЛогированиеТрафика.ЗаписатьИсключенияВРС();
		КонецЕсли;
	...
	Возврат Новый Структура("ДлительнаяОперация, КоличествоИмпортированных", ДлительнаяОперацияИсключенияЖР, КоличествоИмпортированных);
