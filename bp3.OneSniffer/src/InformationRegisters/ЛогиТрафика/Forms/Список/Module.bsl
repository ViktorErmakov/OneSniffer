...
Процедура РедактироватьПароль(Команда)
	...
	Текст = ПроизвольныйЗапрос.ПолучитьТекст();
	...
	Для Каждого СтрокаМассива Из ТекстМассив Цикл
		...
		Если СтрНайти(СтрокаМассива, "Authorization: Bearer ") Тогда
			...
			ШифрованныйТекст = СокрЛП(СтрЗаменить(СтрокаМассива, "Authorization: Bearer ", ""));
			
			ОповещениеОЗакрытии = Новый ОписаниеОповещения("РедактироватьПарольПродолжение", ЭтотОбъект);
