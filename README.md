...
В тексте запроса: переменные `{{$guid}}`, `{{$time}}`, `{{Выражение}}`. Тела смотрите и правьте в Monaco (JSON / REST).

![Редактор запроса](docs/images/editor.png)

### Настройки логирования

Общие настройки логирования (включая курсор импорта из ЖР) хранятся в регистре сведений `OneSniffer_Настройки`. Персональные настройки редактора Monaco — в `ХранилищеОбщихНастроек` (ключ объекта `"OneSniff"`).
...
| **Дата чтения ЖР** | С какой даты читать журнал (поле настроек в РС `OneSniffer_Настройки`) |
| **Срок хранения** | Удаление записей старше N дней. `0` — автоочистка выключена. РЗ `OneSniffer_ОчисткаЛоговТрафика`: гиперссылка расписания и **Выполнить сейчас** |
