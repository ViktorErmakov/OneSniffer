...
Увеличьте `OneSniffer_РедакторКодаКлиентСервер.ВерсияМакетаРедактора()`. EDT: `F5`. Проверка: Список (Запрос/Ответ/произвольный), Просмотр, Настройки редактора (без адреса статики).
...
| HTTP (.rest) / произвольный / лог | `rest` |
